...
function chatsKey(address: string) { return `w3g_chats_${address.toLowerCase()}`; }
function msgsKey(chatId: string) { return `w3g_msgs_${chatId.toLowerCase()}`; }
...
const isDark = _ls('theme') === 'dark' || (!_ls('theme') && window.matchMedia('(prefers-color-scheme: dark)').matches);
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  isNewChatOpen: false,
  isDarkMode: isDark,
  e2e: { isInitialized: false, xmtpReady: false, xmtpAvailable: false, contractsReady: false },
  ...
  setE2EInitialized: (initialized) => set((s) => ({ e2e: { ...s.e2e, isInitialized: initialized } })),
  setXmtpReady: (ready) => set((s) => ({ e2e: { ...s.e2e, xmtpReady: ready } })),
  setXmtpAvailable: (available) => set((s) => ({ e2e: { ...s.e2e, xmtpAvailable: available } })),
  setWallet: (wallet) => {
    set({ wallet });
    if (wallet.address) {
  ...
  toggleNewChat: () => set((s) => ({ isNewChatOpen: !s.isNewChatOpen })),
  markChatAsRead: (chatId) => set((s) => ({
    chats: s.chats.map((c) => c.id === chatId ? { ...c, unreadCount: 0 } : c),
  })),
  pinChat: (chatId) => set((s) => ({
