...
class WalletService {
  ...
  async tryGetAccounts(): Promise<string[]> {
    ...
    // Primary: read from session object
    try {
    ...
    // Also check wcProvider.accounts
    try {
  ...
  async connectFromExistingSession(
    ...
    if (accounts.length === 0) {
      throw new Error('Адрес кошелька не найден в сессии');
    }
    ...
    await Promise.race([
      this._switchToPolygon(provider).catch((e: Error) => {
        console.warn('[WC] Network switch skipped:', e.message);
      }),
  ...
  private async _connectWC(
    ...
    for (let attempt = 1; attempt <= WC_MAX_RETRIES; attempt++) {
      try {
        ...
        if (!isNetworkError || attempt >= WC_MAX_RETRIES || this._aborted) {
          throw err;
        }
    ...
    const initTimeout = new Promise((_, reject) =>
      setTimeout(
        () =>
          reject(
            new Error(
              `WalletConnect не отвечает (${
                attempt > 1 ? 'повтор ' + attempt + ': ' : ''
              }${WC_INIT_TIMEOUT_MS / 1000} сек). Проверьте интернет.`
            )
          ),
        WC_INIT_TIMEOUT_MS
      )
    ...
    const sessionPromise = new Promise<void>((resolve, reject) => {
      ...
      setTimeout(
        () =>
          doReject(
            new Error(
              'Сессия не установлена за 5 минут. Откройте кошелёк и подтвердите.'
            )
          ),
  ...
  private async _clearWCStorage(): Promise<void> {
    try {
      ...
      for (const db of dbs) {
        if (
          db.name &&
          (db.name.includes('WALLET_CONNECT') || db.name.includes('wc@2'))
        ) {
  ...
  private async _switchToPolygon(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (
          ...
          console.warn(`[WC] getNetwork attempt ${attempt + 1} failed, retrying…`);
    ...
    throw lastError || new Error('Не удалось определить сеть');
  }
}
