...
import { X, Copy, Check, ArrowLeft, Smartphone, ExternalLink } from 'lucide-react';
...
export function WalletModal() {
  ...
  const { connect, disconnect, isConnecting, isConnected, address, isCapacitor, hasMetaMask } =
    useWeb3Messenger();
  ...
  const handleConnect = async (type: 'metamask' | 'walletconnect' | 'trust') => {
    try {
      ...
      // Silent: deep-link redirected, user will come back
      console.error(err);
    } finally {
  ...
  // While a deep-link wallet is opening, show "waiting" screen
  const isDeepLinkPending =
    isConnecting && (connectingType === 'metamask' || connectingType === 'trust') && isCapacitor;

  const deepLinkLabel =
    connectingType === 'trust' ? 'Trust Wallet' : 'MetaMask';
                ...
                <div className="space-y-5">
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={copyAddress}
                        className="p-2.5 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-xl transition-colors"
                      >
                        {copied ? (
                  ...
                  <button
                    ...
                    className="w-full py-3.5 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 text-red-500 font-medium rounded-2xl transition-colors"
                  >
                    Отключить кошелёк
                  </button>
                </div>
              ) : isDeepLinkPending ? (
                /* ── Deep-link waiting screen ── */
                <div className="flex flex-col items-center justify-center py-8 space-y-5">
                  ...
                  <div className="text-center space-y-1">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-sm">
                      Подтвердите подключение в {deepLinkLabel}
                    </p>
                    <p className="text-gray-400 dark:text-gray-500 text-xs mt-2">
                      Вернитесь в приложение после подтверждения
                    </p>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-blue-500">
                    <ExternalLink className="w-3.5 h-3.5" />
                    <span>{deepLinkLabel} открыт в отдельном окне</span>
                  </div>
                </div>
              ) : (
                /* ── Wallet picker ── */
                <div className="space-y-5">
                  <div className="text-center pb-2">
                    <p className="text-gray-500 dark:text-gray-400 text-sm max-w-xs mx-auto">
                      {isCapacitor
                        ? 'Выберите кошелёк — он откроется автоматически'
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      <div className="text-left flex-1">
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
                          {isCapacitor ? (
                            <><Smartphone className="w-3 h-3" /> Откроет приложение MetaMask</>
                          ) : hasMetaMask ? (
                            <><span>✓</span> Расширение обнаружено</>
                          ) : (
                    ...
                    >
                      ...
                      <div className="text-left flex-1">
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          QR-код для любого совместимого кошелька
                        </div>
                      </div>
                    ...
                    {/* Trust Wallet — deep-link in APK, WalletConnect in browser */}
                    <button
                    ...
                    >
                      ...
                      <div className="text-left flex-1">
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          {isCapacitor
                            ? 'Откроет приложение Trust Wallet'
                            : 'Через WalletConnect'}
                        </div>
                      </div>
